...
{
	public class spSelectPURCHASE_PROMOTION
	{
		#region Private Members
		private string sp_Name = " spSelectPURCHASE_PROMOTION" ;
		...
		private int m_PURCHASE_PROMOTION_ID;
		...
		public int PURCHASE_PROMOTION_ID
		{
			set
			{
				m_PURCHASE_PROMOTION_ID = value ;
			}
			get
			{
				return m_PURCHASE_PROMOTION_ID;
			}
		}
		...
		#region Constructor
		public spSelectPURCHASE_PROMOTION()
		{
		...
		public bool  ExecuteQuery()
		{
			try
			{
				...
				cmd.CommandText = "spSelectPURCHASE_PROMOTION";
		...
		public IDataReader ExecuteReader()
		{
			try
			{
				...
				command.CommandText = "spSelectPURCHASE_PROMOTION";
		...
		public string ExecuteScalar()
		{
			...
			public void FirstReader(IDataReader dr)
			{
				if(dr.Read())
				{
					...
					m_GST_RATE= float.Parse(Convert.ToDouble(dr["GST_RATE"]).ToString());
					...
					m_PURCHASE_PROMOTION_ID= Convert.ToInt32(dr["PURCHASE_PROMOTION_ID"]);
		    ...
		    public void GetParameterCollection(ref IDbCommand cmd)
		{
			...
			parameter = ProviderFactory.GetParameter(EnumProviders.SQLClient);
			parameter.ParameterName = "@PURCHASE_PROMOTION_ID" ; 
			parameter.DbType = ProviderFactory.GetDBType(EnumProviders.SQLClient, EnumDBTypes.Int);
			if(m_PURCHASE_PROMOTION_ID==Constants.IntNullValue)
			{
				parameter.Value = DBNull.Value;
			}
			else
			{
				parameter.Value = m_PURCHASE_PROMOTION_ID;
			}
			pparams.Add(parameter);
