...
{
	public class spUpdateDTDAMAGED_STOCK
	{
		#region Private Members
		private string sp_Name = " spUpdateDTDAMAGED_STOCK" ;
		private IDbConnection m_connection;
		private IDbTransaction m_transaction;


		private DateTime m_EXPIRY_DATE;
		private int m_DISPOSED_STOCK;
		...
		public int DISPOSED_STOCK
		{
			set
			{
				m_DISPOSED_STOCK = value ;
			}
			get
			{
				return m_DISPOSED_STOCK;
			}
		}
		...
		#region Constructor
		public spUpdateDTDAMAGED_STOCK()
		{
		...
		public bool  ExecuteQuery()
		{
			try
			{
				...
				cmd.CommandText = "spUpdateDTDAMAGED_STOCK";
		...
		public IDataReader ExecuteReader()
		{
			try
			{
				...
				command.CommandText = "spUpdateDTDAMAGED_STOCK";
		...
		public string ExecuteScalar()
		{
			...
			public void FirstReader(IDataReader dr)
			{
				if(dr.Read())
				{
					...
					m_DISPOSED_STOCK= Convert.ToInt32(dr["DISPOSED_STOCK"]);
		    ...
		    public void GetParameterCollection(ref IDbCommand cmd)
		{
			...
			parameter = ProviderFactory.GetParameter(EnumProviders.SQLClient);
			parameter.ParameterName = "@DISPOSED_STOCK" ; 
			parameter.DbType = ProviderFactory.GetDBType(EnumProviders.SQLClient, EnumDBTypes.Int);
			if(m_DISPOSED_STOCK==Constants.IntNullValue)
			{
				parameter.Value = DBNull.Value;
			}
			else
			{
				parameter.Value = m_DISPOSED_STOCK;
			}
			pparams.Add(parameter);
