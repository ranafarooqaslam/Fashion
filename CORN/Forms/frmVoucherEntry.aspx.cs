...
public partial class Forms_frmVoucherEntry : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            RowId = -1;
            DateTime workDate = (DateTime)this.Session["CurrentWorkDate"];
            txtChequeDate.Text = workDate.ToString("dd-MMM-yyyy");
            txtChequeDate.Attributes.Add("readonly", "readonly");
            txtDueDate.Text = workDate.ToString("dd-MMM-yyyy");
            txtDueDate.Attributes.Add("readonly", "readonly");
            txtVoucherDate.Text = workDate.ToString("dd-MMM-yyyy");
        ...
        if (DrpVoucherType.Items.Count > 0)
        {
            ...
            if (int.Parse(DrpVoucherType.SelectedValue.ToString()) == 14)
            {
                ...
                DataTable dt = mAccountController.SelectAccountHead(Constants.AC_AccountHeadId,
                    long.Parse(CORNCommon.Classes.Configuration.CashDefaultType));
            ...
            else if (int.Parse(DrpVoucherType.SelectedValue.ToString()) == 15)
            {
                AccountHeadController mAccountController = new AccountHeadController();
                DataTable dt = mAccountController.SelectAccountHead(Constants.AC_AccountHeadId,
                    long.Parse(CORNCommon.Classes.Configuration.BankDefaultType));
                clsWebFormUtil.FillDropDownList(drpBanks, dt, 0, 4, true);
    ...
    private void LoadAccountDetail()
    {
        ...
        clsWebFormUtil.FillDropDownList(this.drpAccountCodeName, dtHead, "ACCOUNT_HEAD_ID", "ACCOUNT_DETAIL", true);
    ...
    /// <summary>
    /// Sets Voucher For Edit. This Function Runs When An Existing Voucher Needs To Be Edited
    /// </summary>
    /// <param name="sender">object</param>
    /// <param name="e">GridViewEditEventArgs</param>
    protected void GrdOrder_RowEditing(object sender, GridViewEditEventArgs e)
    {
       ...
       // txtAccountCode.Text = GrdOrder.Rows[e.NewEditIndex].Cells[1].Text;
        ...
        drpAccountCodeName.SelectedValue = GrdOrder.Rows[e.NewEditIndex].Cells[0].Text;
        ...
        {
            GrdOrder.Rows[i].Cells[8].Enabled = false;
    ...
    protected void btnDone_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            voucherdate = DateTime.Parse(txtVoucherDate.Text);//(ConvertDate.British_To_American(txtVoucherDate.Text));
                                                              // }

            // if (txtChequeDate.Text.Length == 10)
            // {
            chequeDate = DateTime.Parse((txtChequeDate.Text));// (ConvertDate.British_To_American(txtChequeDate.Text));
            ...
            //  {
            dueDate = DateTime.Parse(txtDueDate.Text);// (ConvertDate.British_To_American(txtDueDate.Text));
        }
        catch (Exception ee)
        {
      ...
      //  }
        dtVoucher = (DataTable)this.Session["dtVoucher"];
        string maxDocumentId = mLController.SelectMaxVoucherId(int.Parse(DrpVoucherType.SelectedValue), int.Parse(drpDistributor.SelectedValue),voucherdate);

        bool isSaveVoucher = mLController.Add_Voucher(int.Parse(drpDistributor.SelectedValue), int.Parse(DrpPrincipal.SelectedValue.ToString()), maxDocumentId, int.Parse(DrpVoucherType.SelectedValue),
               voucherdate, int.Parse(DrpPaymentMode.SelectedValue), Convert.ToString(txtpayeesName.Text), txtRemarks.Text, chequeDate, txtChequeNo.Text, dtVoucher, int.Parse(this.Session["UserId"].ToString()), txtSlipNo.Text, dueDate);
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        DataTable dtHead = (DataTable)this.Session["dtHead"];
        dtVoucher = (DataTable)this.Session["dtVoucher"];
        DataRow[] foundRows = dtHead.Select("ACCOUNT_HEAD_ID  = '" +drpAccountCodeName.SelectedValue + "'");
