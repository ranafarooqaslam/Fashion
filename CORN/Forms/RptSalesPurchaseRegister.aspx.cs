...
public partial class Forms_RptSalesPurchaseRegister : System.Web.UI.Page
{
    ...
    protected void btnPDF_Click(object sender, EventArgs e)
    {
        ...
        {
            CrpReport = new CORNBusinessLayer.Reports.CrpSalesPurchaseTaxableRegister();
        }
        else if (rblReportType.SelectedIndex == 1)
        {
            CrpReport = new CORNBusinessLayer.Reports.CrpSalesPurchaseIIIScheduledRegister();
        }
        else if (rblReportType.SelectedIndex == 2)
        {
            CrpReport = new CORNBusinessLayer.Reports.CrpSalesPurchaseExemptedRegister();
        }
        else if (rblReportType.SelectedIndex == 3)
        {
            CrpReport = new CORNBusinessLayer.Reports.CrpSalesPurchaseTaxablePurchaseRegister();
        }
        else if (rblReportType.SelectedIndex == 4)
        {
            CrpReport = new CORNBusinessLayer.Reports.CrpSalesPurchaseIIIShedulePurchaseRegister();
        }
        else if (rblReportType.SelectedIndex == 5)
        {
            CrpReport = new CORNBusinessLayer.Reports.CrpSalesPurchaseExemptPurchaseRegister();
        }
        else if (rblReportType.SelectedIndex == 6)
        {
            CrpReport = new CORNBusinessLayer.Reports.CrpSalesPurchaseStockRegister();
        }
    ...
    protected void btnExcel_Click(object sender, EventArgs e)
    {
        ...
        DateTime dtTo = new DateTime(dtMonth.Year, dtMonth.Month, 1);
        dtTo = dtTo.AddMonths(1).AddDays(-1);
        string path = CORNCommon.Classes.Configuration.GetAppInstallationPath() + "\\RouteEfficiencyReport.xls";
